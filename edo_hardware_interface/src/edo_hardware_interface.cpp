...
{
    edoHardwareInterface::edoHardwareInterface(ros::NodeHandle& nh) : nh_(nh) {
        init();
    ...
    void edoHardwareInterface::read() {
        for (int i = 0; i < num_joints_; i++) {
            //joint_position_[i] = edo.getJoint(joint_names_[i]).read();
            int a; 
        }
    }
    ...
    void edoHardwareInterface::write(ros::Duration elapsed_time) {
        ...
        for (int i = 0; i < num_joints_; i++) {
            //edo.getJoint(joint_names_[i]).actuate(joint_effort_command_[i]);
            int b;
        }
    }
}